...
- Pull 连接器**：分页（`page_page_size` / **offset_limit** / **cursor** / **link_header**）、增量水位（**timestamp** / **monotonic_id**）、Transform 映射、试请求与 Schema 推断
...
- **示例模板**（8 个）：`rest-pagination`、`rest-offset-limit`、`rest-cursor`、`rest-link-header`、`rest-monotonic-id`、`rest-rolling-window`、`rest-kafka`、`webhook-json-array`
